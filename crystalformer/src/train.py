...
def train(key, optimizer, opt_state, loss_fn, params, epoch_finished, epochs, batchsize, train_data, valid_data, path):
    ...
    def update(params, key, opt_state, data):
        G, L, X, A, W, S, I = data
        value, grad = jax.value_and_grad(loss_fn, has_aux=True)(params, key, G, L, X, A, W, S, I, True)
    ...
    for epoch in range(epoch_finished+1, epochs+1):
        ...
        _, train_L, _, _, _, _, _ = train_data
        ...
        for batch_idx in range(num_batches):
            ...
            data = jax.tree.map(lambda x: x[start_idx:end_idx], train_data)
            ...
            train_loss, train_aux = jax.tree.map(   
        ...
        if epoch % 100 == 0:
            _, valid_L, _, _, _, _, _ = valid_data 
            valid_loss = 0.0 
            ...
            for batch_idx in range(num_batches):
                start_idx = batch_idx * batchsize
                end_idx = min(start_idx + batchsize, num_samples)
                data = jax.tree.map(lambda x: x[start_idx:end_idx], valid_data)
                data = jax.tree.map(lambda x: x.reshape(shape_prefix + x.shape[1:]), data)
