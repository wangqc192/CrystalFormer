...
def get_validity(crys):
    ...
    return {'comp_valid': 0,
            ...
            'valid': 0,
            'each_valid': [c.struct_valid for c in crys]}
...
def main(args):
    ...
    crys = Parallel(args.num_io_process, backend="multiprocessing")(
                delayed(get_crystal)(literal_eval(cif)) for cif in tqdm(cif_strings)
                    )
    print(len(crys))
